...
def start():
    ...
    ap = GooeyParser()
    ap.add_argument('Type', help="type of dalton", widget='Listbox', choices=['Deuteranope', 'Protanope', 'Tritanope'],
                    nargs='*')
    ap.add_argument('Port', help="port of camera", widget='Listbox', choices=working_ports, nargs='+')
    ...
    for col_deficit in args['Type']:
        ...
        while rval:
            rval, frame = cam.read()
            start = time.time()
            file = execute(numpy.asarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), dtype=float),
                           colorblindness[f'{col_deficit}'])
